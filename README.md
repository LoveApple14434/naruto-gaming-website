...
- **公告系统** — 管理员发布公告，支持 Markdown 内容（含图片宽度控制），可置顶、控制发布状态
...
- **名人堂** — 历届明星选手展示、管理员管理
- **贡献者名单** — 为网页建设提供资金支持的伙伴展示页面，管理员可管理
...
│   │   ├── routes/           # 路由（auth、brackets、bets、checkins、contributors 等）
...
| 日期 | 变更内容 |
|------|----------|
| 2026-07-29 | 公告置顶功能：支持将重要公告置顶显示，置顶公告在列表前排展示并高亮；Markdown 图片支持自定义宽度（`![alt](url =300x)` 语法及 HTML `<img width>`） |
| 2026-07-29 | 新增「网页建设贡献者」页面，用于展示为平台提供资金支持的伙伴，支持管理端 CRUD |
| 2026-07-29 | 公告图片懒加载 404 修复：SPA 切页时自动补全 `/naruto` 前缀，确保图片路径正确 |
| 2026-07-29 | 新增修改密码功能（需验证旧密码），引入 `tokenVersion` 机制，改密码后旧 JWT 令牌立即失效，安全性提升 |
| 2026-07-29 | 竞猜取消功能：管理员可取消已发布的竞猜（软删除 `cancelled` 标记），自动退还所有用户已投注的竞猜币；修复部署脚本数据库同步顺序，避免新字段导致 500 错误 |
